...
from src.utils.bricks import run_time
from src.utils.vis_utils import visualize_geomxyz
import matplotlib.pyplot as plt

__all__ = ['BaseVAMPIRE2']
...
class BaseVAMPIRE2(nn.Module):
    ...
    def __init__(self,
        ...
        super(BaseVAMPIRE2, self).__init__()
        ...
        self.register_buffer('occ_coords', self.create_norm_occ_coords(norm=False))
        # self.register_buffer('norm_occ_coords', self.create_norm_occ_coords(norm=True))
    ...
    def create_norm_occ_coords(self, point_cloud_range=[-40.0, -40.0, -1.0, 40.0, 40.0, 5.4], voxelSize=[0.4, 0.4, 0.4], norm=False):
        ...
        if not norm:
            return occ_coords.reshape(200, 200, 16, 3)
    ...
    def _forward_single_sweep(self,
        ...
        depth_softmax_features = self.mapping_along_depth(source_features).softmax(dim=1).reshape(batch_size, num_cams, -1, img_feats.shape[-2], img_feats.shape[-1])
        low_channel_source_features = self.channel_lower(source_features).reshape(batch_size, num_cams, -1, img_feats.shape[-2], img_feats.shape[-1])
        ...
        bda_mat = mats_dict.get('bda_mat', None)[:, :3, :3]
        bda_mat = bda_mat.view(batch_size, 1, 1, 1, 3, 3)

        occ_coords = (bda_mat @ self.occ_coords[None, ..., None].expand(batch_size, *self.occ_coords.shape, 1)).squeeze(-1)
        norm_occ_coords = (occ_coords - torch.as_tensor([self.x_bound_seg[0], self.y_bound_seg[0], self.z_bound_seg[0]],
                                                             device=self.camera_mids.device)) / torch.as_tensor(
                    [self.x_bound_seg[1] - self.x_bound_seg[0], self.y_bound_seg[1] - self.y_bound_seg[0],
                     self.z_bound_seg[1] - self.z_bound_seg[0]], device=self.camera_mids.device)
        norm_occ_coords = norm_occ_coords * 2. - 1.
        # norm_occ_coords = self.norm_occ_coords
        occ_logits = F.grid_sample(semantic_logits, norm_occ_coords, padding_mode='border', align_corners=True)
        occ_density = F.grid_sample(self.density(density_feature), norm_occ_coords, align_corners=True)
