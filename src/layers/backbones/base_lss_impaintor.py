...
class BaseLSSImpaintor(nn.Module):
    ...
    def __init__(self,
        ...
        super(BaseLSSImpaintor, self).__init__()
        ...
        self.register_buffer('voxel_coords',
                             self.create_voxel_coords(self.x_bound_seg, self.y_bound_seg, self.z_bound_seg))
        ...
        if self.cat_pos:
            self.register_buffer('norm_voxel_coords',
                                 self.create_voxel_coords(self.x_bound_seg, self.y_bound_seg, self.z_bound_seg,
                                                          norm=True))
        self.register_buffer('output_coords',
                             self.create_voxel_coords(self.x_bound_det, self.y_bound_det, self.z_bound_det))
        ...
        self.mapping_along_depth = nn.Sequential(
            nn.Conv2d(img_out_channels, self.depth_channels, 3, 1, 1, bias=False),
            # nn.ReLU(),
            # nn.Conv2d(self.mid_channels, self.depth_channels, 3, 1, 1, bias=False),
        )
        ...
        # )
        self.base_conv = Unet3D(voxel_in_channels, self.mid_channels)
        ...
        if self.oY == 128:
            self.voxel_output = nn.Conv2d(voxel_out_in * self.output_coords.shape[0], self.output_channels, 1, 1,
                                          bias=True)
        elif self.oY == 256:
        ...
        for m in self.density_conv.modules():
            if isinstance(m, nn.Conv3d):
                n = m.kernel_size[0] * m.kernel_size[1] * m.kernel_size[
                    2] * m.out_channels
                bound = 1 / math.sqrt(n)
                # nn.init.uniform_(m.bias, self.sdf_bias - bound, self.sdf_bias + bound)
                nn.init.constant_(m.bias, self.sdf_bias - 10.0)
    ...
    def create_bev_mids(self):
        tdist = torch.linspace(self.z_bound_det[0] + self.z_bound_det[2] / 2.,
                               self.z_bound_det[1] - self.z_bound_det[2] / 2.,
                               int((self.z_bound_det[1] - self.z_bound_det[0]) / self.z_bound_det[2]),
                               dtype=torch.float)  # - self.z_bound[0]
    ...
    def create_norm_occ_coords(self, point_cloud_range=[-40.0, -40.0, -1.0, 40.0, 40.0, 5.4],
        ...
        norm_occ_coords = (occ_coords - torch.as_tensor(
    ...
    def volume_rendering_from_multiple_views(self, geom_xyz, density_feature, semantic_logits, voxel_features, rgb):
        ...
        frustum_features = F.grid_sample(temp_fusion_features, norm_geom_xyz.reshape(batch_size, -1, h, w, 3),
                                         align_corners=True)
        frustum_features = frustum_features.reshape(batch_size, -1, num_cams, d - 1, h, w).permute(0, 2, 1, 3, 4,
                                                                                                   5) * geom_valid_mask.unsqueeze(
            2)
    ...
    def get_voxel_feats(self, frustum_feats, sweep_index, mats_dict, clamp_extreme=True):
        batch_size, num_cams, num_channels, d, h, w = frustum_feats.shape
        ...
        z_valid = (z > self.d_bound[0]).bool() & (z < self.d_bound[1]).bool()
        valid = (x_valid & y_valid & z_valid).float()

        norm_x = 2.0 * (x / float(self.final_dim[1] - 1)) - 1.0
        norm_y = 2.0 * (y / float(self.final_dim[0] - 1)) - 1.0
        norm_z = 2.0 * ((z - self.d_bound[0]) / (self.d_bound[1] - self.d_bound[0])) - 1.0
        if clamp_extreme:
            norm_x = torch.clamp(norm_x, min=-2.0, max=2.0)
            norm_y = torch.clamp(norm_y, min=-2.0, max=2.0)
            norm_z = torch.clamp(norm_z, min=-2.0, max=2.0)
        norm_xyz = torch.stack([norm_x, norm_y, norm_z], dim=-1).reshape(-1, self.vZ, self.vY, self.vX, 3)
        voxel_features = F.grid_sample(frustum_feats.reshape(-1, num_channels, d, h, w), norm_xyz, align_corners=False)
        voxel_features = voxel_features.reshape(batch_size, num_cams, num_channels, self.vZ, self.vY,
                                                self.vX) * valid.unsqueeze(2)
    ...
    def _forward_single_sweep(self,
        ...
        source_features = img_feats[:, 0, ...].reshape(batch_size * num_cams, -1, img_feats.shape[-2],
                                                       img_feats.shape[-1])
        depth_softmax_features = self.mapping_along_depth(source_features).softmax(dim=1).reshape(batch_size, num_cams,
                                                                                                  -1,
                                                                                                  img_feats.shape[-2],
                                                                                                  img_feats.shape[-1])
        low_channel_source_features = self.channel_lower(source_features).reshape(batch_size, num_cams, -1,
                                                                                  img_feats.shape[-2],
                                                                                  img_feats.shape[-1])
        img_feats_with_depth = depth_softmax_features.unsqueeze(2) * low_channel_source_features.unsqueeze(3)
        ...
        voxel_features = self.get_voxel_feats(img_feats_with_depth, sweep_index, mats_dict)  # 1,32,20,256,256
        ...
        rgb = self.rgb_conv(base_features)
        ...
        if inrange_pts is not None:
            for i in range(batch_size):
                ...
                norm_pts = (inrange_pts[i] - torch.as_tensor(
                    [self.x_bound_seg[0], self.y_bound_seg[0], self.z_bound_seg[0]],
                    device=self.camera_mids.device)) / torch.as_tensor(
                ...
                pts_logits = F.grid_sample(semantic_logits[[i], ...], norm_pts, padding_mode='border',
                                           align_corners=True)
        ...
        occ_logits = F.grid_sample(semantic_logits,
                                   self.norm_occ_coords[None, ...].expand(batch_size, *self.norm_occ_coords.shape),
                                   padding_mode='border', align_corners=True)
        occ_density = F.grid_sample(self.density(density_feature),
                                    self.norm_occ_coords[None, ...].expand(batch_size, *self.norm_occ_coords.shape),
                                    align_corners=True)
        ...
        rgb_preds, seg_logits_preds, depth_preds, bev_rgb_preds, bev_seg_logits_preds, bev_height_preds, bev_density, voxel_output = \
            self.volume_rendering_from_multiple_views(geom_xyz, density_feature, semantic_logits, base_features, rgb)
        ...
        # voxel_output_features = voxel_output.sum(2)
        # voxel_output_features = F.interpolate(voxel_output_features, scale_factor=0.5, mode='bilinear',
        #                                       align_corners=True)
...
class hourglass3D(nn.Module):
    def __init__(self, in_channels, mid_channels):
        ...
        self.conv1 = nn.Sequential(nn.Conv3d(in_channels, mid_channels * 2, kernel_size=3, padding=1, stride=2, bias=False),
                                   nn.ReLU(inplace=True))

        self.conv2 = nn.Conv3d(mid_channels * 2, mid_channels * 2, kernel_size=3, padding=1, stride=2, bias=False)

        self.conv3 = nn.Sequential(nn.Conv3d(mid_channels * 2, mid_channels * 2, kernel_size=3, padding=1, stride=2, bias=False))

        self.conv4 = nn.Sequential(nn.Conv3d(mid_channels * 2, mid_channels * 2, kernel_size=3, padding=1, stride=2, bias=False),
                                   nn.ReLU(inplace=True))

        self.conv5 = nn.Sequential(
            nn.ConvTranspose3d(mid_channels * 2, mid_channels * 2, kernel_size=3, padding=1, output_padding=1, stride=2,
                               bias=False))  # +conv2

        self.conv6 = nn.Sequential(
            nn.ConvTranspose3d(mid_channels * 2, mid_channels, kernel_size=3, padding=1, output_padding=1, stride=2,
                               bias=False))  # +x
...
class HG3D(nn.Module):
    def __init__(self, in_channels, mid_channels):
        super(HG3D, self).__init__()

        self.conv1 = nn.Sequential(nn.Conv3d(in_channels, mid_channels * 2, kernel_size=3, padding=1, stride=2, bias=False),
                                   nn.ReLU(inplace=True))

        self.conv2 = nn.Sequential(nn.Conv3d(mid_channels * 2, mid_channels * 2, kernel_size=3, padding=1, stride=2, bias=False),
                                   nn.ReLU(inplace=True))

        self.conv3 = nn.Sequential(nn.Conv3d(mid_channels * 2, mid_channels * 2, kernel_size=3, padding=1, stride=1, bias=False),
                                   nn.ReLU(inplace=True))

        self.conv4 = nn.Sequential(nn.Conv3d(mid_channels * 2, mid_channels * 2, kernel_size=3, padding=1, stride=1, bias=False),
                                   nn.ReLU(inplace=True))

        self.conv5 = nn.Sequential(nn.Conv3d(mid_channels * 2, mid_channels, kernel_size=3, padding=1, stride=1, bias=False))
        # self.conv5 = nn.Sequential(
        #     nn.ConvTranspose3d(mid_channels * 2, mid_channels * 2, kernel_size=3, padding=1, output_padding=1, stride=2,
        #                        bias=False))  # +conv2
        #
    ...
    def forward(self, x):
        ...
        pre1 = self.conv1(x)  # 1 64 10 128 128
        pre2 = self.conv2(pre1)  # 1 64 5 64 64

        mid = self.conv3(pre2)  # 1 64 5 64 64
        # out = self.conv4(out)  # in:1/16 out:1/16

        post2 = F.relu(mid + pre2, inplace=True)  # 1 64 5 64 64

        post1 = self.conv4(F.interpolate(post2, (pre1.shape[-3], pre1.shape[-2], pre1.shape[-1]), mode='trilinear', align_corners=True) + pre1)  # 1 64 10 128 128

        out = F.interpolate(post1, (x.shape[-3], x.shape[-2], x.shape[-1]), mode='trilinear', align_corners=True)  # 1 64 20 256 256
        out = self.conv5(out)
